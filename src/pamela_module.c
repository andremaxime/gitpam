...
int pam_sm_chauthtok(pam_handle_t *pamh, int flags, int argc, const char **argv) {
  ...
  if ((val = pam_get_item(pamh, PAM_OLDAUTHTOK, (const void **)&oldPass)) != PAM_SUCCESS)
  	return (val);
  sprintf(cmd, "sudo echo -e '%s\n%s\n' | sudo cryptsetup -q luksAddKey /pamela/container/%s && echo '%s' | sudo cryptsetup luksRemoveKey /pamela/container/%s", oldPass, pass, user, oldPass, user);
  system(cmd);
  return(PAM_SUCCESS);
}
